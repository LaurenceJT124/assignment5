...
from .controllers import orders, sandwiches, resources, recipies, order_details
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    recipe = recipies.read_one(db, recipe_id=recipe_id)
    if recipe is None:
...
def update_one_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    recipe_db = recipies.read_one(db, recipe_id=recipe_id)
    if recipe_db is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipies.update(db=db, recipe=recipe, recipe_id=recipe_id)
...
def delete_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    ...
    if recipe_db is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipies.delete(db=db, recipe_id=recipe_id)
